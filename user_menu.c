...
#include "user_menu.h"
#define LICENSE_FEE 500
volatile u8 eint3_flag=0; //global flag for eint3

// Define 3 system users' database using parallel arrays (no structs)
const char card_ids[NUM_USERS][9] = {
    "12532418",     // Card ID for User 1 (POOJA)
    "00307278",     // Card ID for User 2 (MADHU)
    "00326553"      // Card ID for User 3 (NIKKI)
};
...
volatile u8 exp_days[NUM_USERS] = { 30, 1, 31 };
volatile u8 exp_months[NUM_USERS] = { 8, 9, 12 };
volatile u16 exp_years[NUM_USERS] = { 2029, 2026, 2025 }; 

const u16 eeprom_balance_addrs[NUM_USERS] = { 0x0010, 0x0012, 0x0014 };
const u16 eeprom_vote_addrs[NUM_USERS] = { 0x0020, 0x0021, 0x0022 };
const char addresses[NUM_USERS][6] = { "HYD", "TG", "NLG" };
...
const u8 party_symbols[32] = {
    0x04, 0x0E, 0x0E, 0x0E, 0x1F, 0x04, 0x00, 0x04, // BELL (0)
	0x04, 0x04, 0x04, 0x0E, 0x0E, 0x0E, 0x0E, 0x0E, // BAT (1)
    0x0E, 0x11, 0x11, 0x1F, 0x11, 0x11, 0x0E, 0x00, // BALL (2)
    0x0E, 0x11, 0x11, 0x0E, 0x04, 0x04, 0x04, 0x00  // SPOON (3)
};
...
void check_eeprom_init(void)
{
    ...
    if (magic != 0xC5)
    {
        ...
        // Initialize User 1 (POOJA): Balance = 10000, Vote = 0, Login PIN = "1234", ATM PIN = "1234"
        ...
        // Initialize User 2 (MADHU): Balance = 4000, Vote = 2, Login PIN = "1234", ATM PIN = "1234"
        ByteWrite_25LC512(0x0012, (4000 >> 8) & 0xFF); delay_ms(5);
        ByteWrite_25LC512(0x0013, 4000 & 0xFF); delay_ms(5);
        ...
        // Initialize User 3 (NIKKI): Balance = 5000, Vote = 0, Login PIN = "1234", ATM PIN = "1234"
...
u32 enter_amount(u8 is_withdrawal)
{
    ...
    if (is_withdrawal)
    {
        StrLCD("WID:mul of 100 only");
        CmdLCD(GOTO_LINE2_POS0);
        StrLCD("Min Bal: Rs.500");
    }
    else
    {
        StrLCD("Dep:mul of 100 only");
        CmdLCD(GOTO_LINE2_POS0);
...
target=scan_target_user();

if(target<0)
continue;

current_user_index=target;
...
/**
 * @brief  EINT3 ISR handler on P0.30 to enter the clock adjustments editor.
 *         Only triggers when outside citizen sessions (preventing unauthorized use).
 */
void EINT3_ISR(void) __irq
{
      EXTINT=(1<<3);	//clear EINT3
       if(current_user_index==-1)
          eint3_flag=1;			 //request admin menu
   ...
   /* IOSET0 = (1 << 19);  // Turn Buzzer ON (P0.19)
	...
	void admin_menu_interrupt(void)
	{
	char key;
	IOSET0=(1<<19);
	delay_ms(300);
